...
Blockly.BlockSvg.prototype.renderFields_ = function(fieldList,
  ...
  var hasIcon = fieldList[0] instanceof Blockly.FieldImage ;
  console.log(hasIcon) ;
  for (var t = 0, field; field = fieldList[t]; t++) {
    ...
    if (this.RTL) {
      ...
      if(hasIcon&&t>0){//有图片icon的情况
        if(t==1&&fieldList[1] instanceof Blockly.FieldImage){
          root.setAttribute('transform',
          'translate(' + (cursorX + field.renderSep-10) + ',' + cursorY + ')');
        }else{
          root.setAttribute('transform',
            'translate(' + (cursorX + field.renderSep-10) + ',' + (cursorY+10) + ')');
        }
      }else{
        root.setAttribute('transform',
